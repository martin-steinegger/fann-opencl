...
/* Function: fann_create_train
   Creates an empty uninitalized training data struct.
   Use <fann_set_train> to initialize.
  
   See also:
     <fann_set_train>, <fann_read_train_from_file>, <fann_train_on_data>, <fann_destroy_train>,
     <fann_save_train>
...
/* Function: fann_set_train
   Sets the input and desired output values into the specified position in the training data structure.

   If you e.g. have two training data samples you would create and set the training data like this.

   data = <fann_create_train>(2, 3, 1);
   <fann_set_train>(data, 0, input0, output0);
   <fann_set_train>(data, 1, input1, output1);
  
   Parameters:
     data          - The training data
	 position      - The position of that the training sample should be set at
     input         - The input data to the training sample as an array of size <fann_num_input_train_data>(data)
     output        - The output data to the training sample as an array of size <fann_num_output_train_data>(data)

   See also:
     <fann_create_train>, <fann_get_train>, <fann_train_on_data>, <fann_destroy_train>,
     <fann_save_train>
...
/* Function: fann_get_train
   Gets the input and desired output values from the specified position in the training data structure.

   Parameters:
     data          - The training data
	 position      - The position of that the training sample should be taken from
     input         - The input data which the training data will be returned into as an array of size <fann_num_input_train_data>(data)
     output        - The output data which the training data will be returned into as an array of size <fann_num_output_train_data>(data)
